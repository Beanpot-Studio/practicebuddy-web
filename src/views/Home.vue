<template web>
	<main class="column is-four-fifths main">
		<div class="box is-radiusless is-shadowless has-background-light">
			...
			<div v-if="teacher != null">Your teacher is: {{ teacher.name }}</div>
			<div v-else>
				<span v-if="students.length == 0"
					>There is no teacher associated to this account. If you're a student and want to send practice
					information to your teacher, search for your teacher in the 'My Teacher' tab to associate him or her
					to this account.</span
				>
			</div>
		</div>
								...
								<div class="content">
									<article class="media">
										<div class="media-left">
											<figure class="circle">
												<img
													:src="'./instruments/' + student.instrument + '.png'"
													alt="my instrument"
												/>
											</figure>
										</div>
		...
		<div class="box main-content" v-else>
			...
			<h3 v-if="practices.length == 0">Student practice sessions will be displayed here</h3>
									...
									<article class="media">
										...
										<div>
											<div class="heading has-text-white">
												{{ practice.updated.seconds | moment('MMMM Do YYYY, h:mm:ss a') }}
											</div>
											<div
											...
											<div class="has-text-white is-size-4">
												{{ practice.practicelength }} minutes
											</div>
											<div v-if="practice.feedback" class="has-text-white is-size-5">
												Teacher feedback: {{ practice.feedback }}
											</div>
										</div>
									</article>
								</div>
